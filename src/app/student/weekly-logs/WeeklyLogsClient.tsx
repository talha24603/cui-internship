...
export default function WeeklyLogsClient() {
  ...
  const weekHighlight = searchParams.get("week");

  useEffect(() => {
    if (!weekHighlight) return;
    const id = `weekly-log-${weekHighlight}`;
    const t = window.setTimeout(() => {
      document.getElementById(id)?.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 100);
    return () => window.clearTimeout(t);
  }, [weekHighlight, data?.weeklyLogs]);
          ...
          <div className="space-y-3">
            {initialLoading ? (
              ...
              data.weeklyLogs.map((log) => (
                <div
                  key={log.id}
                  id={`weekly-log-${log.weekNo}`}
                  className={`rounded-lg border p-3 ${
                    weekHighlight === String(log.weekNo)
                      ? "border-sky-400 bg-sky-50/80 ring-2 ring-sky-200 dark:border-sky-600 dark:bg-sky-950/40 dark:ring-sky-800"
                      : "border-slate-200 dark:border-slate-700"
                  }`}
                >
                  <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">Week {log.weekNo}</p>
                  <p className="text-xs text-slate-700 dark:text-slate-300">Activities: {log.activitiesDone}</p>
                  <p className="text-xs text-slate-700 dark:text-slate-300">Skills: {log.skillsLearned}</p>
                  <p className="text-xs text-slate-700 dark:text-slate-300">Challenges: {log.challenges}</p>
                </div>
              ))
            ) : (
