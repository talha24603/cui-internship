...
const navItems = [
  ...
  { href: "/site/internships", label: "Internships", icon: Users },
  { href: "/site/evaluations", label: "Evaluations", icon: ClipboardCheck },
];
...
export default function SiteShell({ title, description, children }: SiteShellProps) {
  ...
  useEffect(() => {
    ...
    if (current.user.role !== "SITE_SUPERVISOR") return router.replace("/dashboard");
    setSession(current);
  }, [router]);

  const activePath = useMemo(() => pathname || "/site", [pathname]);
      ...
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[260px_1fr]">
        ...
        >
          <div className="mb-5 flex items-center gap-3 border-b border-slate-200 pb-4 dark:border-slate-800">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">Site Supervisor</p>
